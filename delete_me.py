...
z_limit = standard_dviations * w_0 / np.cos(alpha_cavity)
Z = np.linspace(-z_limit, z_limit, 1000)
...
def rotated_gaussian_beam_A(x: [float, np.ndarray],
    ...
    A_1 = gaussian_beam(x=x_tilde, y=y, z=z_tilde, E=E_1, lambda_laser=l_1, NA=NA, t=t, mode="potential")
    A_2 = gaussian_beam(x=x_tilde, y=y, z=z_tilde, E=E_2, lambda_laser=l_2, NA=NA, t=t, mode="potential")
...
def phi_integrand(z_idx,
    ...
    G_dz = G_discrete_integral(z_idx, A_z_integrand_values_dz) + A_z_integrand_values_dz[z_idx] * dr
