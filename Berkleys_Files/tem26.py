    ...
    **kwargs :
    ...
    if len(argv) == 0:
        plot_features = {
            ...
            "plot_noisy_image_TF": False,
            "cmap_limits": None,
            "plot_downsampled_image_TF": False,
            "downsample_factor": 5,
            "plot_lpp_TF": False,
            "lpp_zoom_factor": 5,
            "plot_bfp_TF": False,
            "bfp_zoom_factor": 3,
            "plot_asd_log10_TF": False,
            "asd_quantiles": [0, 1],
            "plot_ctf_TF": False,
            "astig_avg_TF": False,
            "ctf_bins": 50,
...
def do_microscopy(img, sample, laser, camera, scope, sim_features=default_sim_features):
    ...
    if not sim_features["load_exit_wave"]:
        ...
        if sim_features["save_exit_wave_TF"]:
            ...
            exit_wave_op = get_exit_wave_op(sample, psi_in, scope, camera, return_potential=True)
        time_stamps = stamp_time("computed_exit_wave", time_stamps)

        if sim_features["save_exit_wave_TF"]:
            np.savez(
                os.path.join(sim_features["save_folder_name"], sim_features["exit_wave_file_name"] + ".npz"),
                sample=sample,
                psi_in=psi_in,
                scope=scope,
                camera=camera,
    ...
    if sim_features["save_results_TF"]:
        np.savez(
            os.path.join(sim_features["save_folder_name"], sim_features["output_file_name"] + ".npz"), results=results
        )
        print(
            "...Saved everything to: "
            + os.path.join(sim_features["save_folder_name"], sim_features["output_file_name"] + ".npz")
        )
...
def plot_results(results, plot_features=default_plot_features):
    ...
    if plot_features["plot_bfp_TF"]:
        plot_bfp(results["laser"], results["camera"], results["scope"], plot_features)
        save_plot(plot_features, "bfp_summary")
    if plot_features["plot_lpp_TF"]:
        plot_lpp(results["laser"], results["camera"], results["scope"], plot_features)
    ...
    if plot_features["plot_noisy_image_TF"]:
        img_noisy = pdf_to_image(
            results["img_ip_pdf"], results["scope"]["dose"], results["camera"]["pixel_size"], noise_flag=True
        )
    ...
    if plot_features["plot_downsampled_image_TF"]:
        ...
        ds_img_noisy = downsample_image(img_noisy, plot_features["downsample_factor"])
        plot_image(ds_img_noisy, plot_features)
        save_plot(plot_features, "noisy_image_downsampled")
    if plot_features["plot_ctf_TF"]:
        plot_ctf(results["img"], results["laser"], results["camera"], results["scope"], plot_features)
...
z_dict = {"H": 1, "C": 6, "N": 7, "O": 8, "NA": 11, "MG": 12, "P": 15, "S": 16, "FE": 26, "AU": 79}  # atomic numbers

# van der waals radii (-1: unknown), en.wikipedia.org/wiki/Van_der_Waals_radius
vdw_dict = {"H": 1.1, "C": 1.7, "N": 1.55, "O": 1.52, "NA": 2.27, "MG": 1.73, "P": 1.8, "S": 1.8, "FE": -1, "AU": 1.66}
...
# get_struct_file_loc
protein_library = ["hemoglobin", "proteasome_human", "proteasome_ta", "apoferritin", "ribosome", "myoglobin"]
...
def get_deriv_params(flag, dic):
    if flag == "scope":
        ...
        dic["wlen"] = const["e_compton"] / (dic["gamma_lorentz"] * dic["beta_lattice"])  # wavelength [A]
        dic["sigma_e"] = (
            2
            * np.pi
            / (dic["wlen"] * dic["HT"])
        ...
        if dic["name"] == "test_plane":
            ...
            raise Exception(
                "currently supported sample names (as string):\n"
                + "test_plane\n"
                + "test_circle\n"
                + "test_circle_array\n"
                + "random_carbon\n"
...
def make_laser_coords_L(S, scope, laser):
    ...
    Rx_rot = Sx_phys * np.cos(np.deg2rad(laser["xy_angle"])) + Sy_phys * np.sin(np.deg2rad(laser["xy_angle"]))  # [A]
    Ry_rot = -Sx_phys * np.sin(np.deg2rad(laser["xy_angle"])) + Sy_phys * np.cos(np.deg2rad(laser["xy_angle"]))  # [A]
...
def make_defocus_coord(S, img):
    ...
    return f_0 + f_a * np.cos(2 * (phi - np.deg2rad(img["astig_angle"])))  # total defocus [A]


#%% transfer function calculations
def get_gamma(S, scope, img):
    f_tot = make_defocus_coord(S, img)  # total defocus (mean + astigmatism) [A]
    return np.pi * (-f_tot * scope["wlen"] * S["r"] ** 2 + 0.5 * scope["Cs"] * scope["wlen"] ** 3 * S["r"] ** 4)
...
def get_gamma_gradient(S, scope, img):
    ...
    dgamma_dSx = (
        ...
        * np.pi
        * (
            (scope["Cs"] * scope["wlen"] ** 3 * S["r"] ** 3 - img["mean_z"] * scope["wlen"] * S["r"]) * np.cos(phi)
            - img["astig_z"] * scope["wlen"] * S["r"] * np.cos(phi - 2 * np.deg2rad(img["astig_angle"]))
        )
    )
    dgamma_dSy = (
        2
        * np.pi
        * (
            (scope["Cs"] * scope["wlen"] ** 3 * S["r"] ** 3 - img["mean_z"] * scope["wlen"] * S["r"]) * np.sin(phi)
            + img["astig_z"] * scope["wlen"] * S["r"] * np.sin(phi - 2 * np.deg2rad(img["astig_angle"]))
        )
    )
...
def get_defocus_spread(scope):
    ...
    return scope["Cc"] * np.sqrt(
        (epsilon * scope["energy_spread_std"] / scope["HT"]) ** 2
        + (epsilon * scope["accel_voltage_std"]) ** 2
        + 4 * (scope["OL_current_std"]) ** 2
    )  # [A]


def get_dqe_envelope(S, camera):
    env_dqe_raw = np.polynomial.polynomial.polyval(S["r"] / camera["nyquist"], camera["dqe_coeffs"])
...
def get_transfer_func_mag_and_phase(chi, S, L, img, laser, camera, scope):
    ...
    if scope["enable_phase_plate_coherence"]:
        if laser["true_zernike_mode"]:  # true zernike phase plate
            ...
            (deta_dSx, deta_dSy) = get_numerical_phase_plate_gradient(laser, camera, scope)
            ...
            if laser["crossed_lpps"]:  # two laser phase plates crossed at 90 deg
                ...
                if laser["crossed_lpp_phis"] is not None:
                    curr_phis = [laser["crossed_lpp_phis"][0], laser["crossed_lpp_phis"][1]]  # [deg]
                ...
                for las_ind in [0, 1]:
                    ...
                    if curr_laser["pol_angle"] == "rra":
                        ...
                        print(
                            "Laser #{} polarization set to ".format(las_ind + 1)
                    ...
                    curr_laser.update({"peak_phase_deg": laser["crossed_lpp_phases_deg"][las_ind]})  # [deg]
                    curr_eta0 = get_eta0_from_peak_phase_deg(curr_laser, L, scope, curr_phis[las_ind])  # [rad]
                    curr_laser.update({"eta0": curr_eta0})

                    # create custom laser coordinate system
                    curr_L = make_laser_coords_L(S, scope, curr_laser)  # [dimensionless]

                    # calculate gradient of this laser
                    (curr_dedx, curr_dedy) = get_laser_phase_gradient(curr_L, curr_laser, scope)
    ...
    A = np.exp(
        -(sigma_s**2) * (V1_x**2 + V1_y**2) / 2
        - sigma_f**2 * (V2**2 - (sigma_s**2 * V1_dot_V3) ** 2) / (2 * (1 + E))
    ) / np.sqrt(1 + E)
...
def get_transfer_func(img, scope, camera, laser, ctf_mode=False, return_ctf_components=False):
    ...
    if scope["coherence_envelopes_on"]:
        (A, xi) = get_transfer_func_mag_and_phase(chi, S, L, img, laser, camera, scope)
    else:
        A = np.ones_like(S["r"])
        xi = -chi
    ...
    if ctf_mode:
        ...
        (nlaser, nL) = get_laser_phase(nlaser, scope, camera, const, return_L=True, negative_flag=True)  # eta(-s)
        nchi = ngamma + nlaser["eta"]  # chi(-s) = gamma_lattice(-s) + eta(-s)
        if scope["coherence_envelopes_on"]:
            # get A(-s), xi(-s)
            (nA, nxi) = get_transfer_func_mag_and_phase(nchi, nS, nL, img, nlaser, camera, scope)
        else:
            nA = np.ones_like(S["r"])
            nxi = -nchi
...
def get_laser_phase(laser, scope, camera, const, return_L=False, return_eta=False, S=None, negative_flag=False):
    if S is None:  # make back focal plane coordinates
        if negative_flag:  # useful for ctf calculation
            ...
            if diagnostics_on:
                if not silent_mode:
                    print(
                        "DIAGNOSTICS: Laser plane pixel spacing is "
                        + "{:.2f} nm".format(
                            scope["wlen"] * scope["focal_length"] / (10 * camera["rows"] * camera["pixel_size"])
                        )
                        + " along rows and "
                        "{:.2f} nm".format(
                            scope["wlen"] * scope["focal_length"] / (10 * camera["rows"] * camera["pixel_size"])
    ...
    if laser["true_zernike_mode"]:  # make circular phase plate
        ...
        if laser["crossed_lpps"]:  # two LPPs at 90 degrees
            ...
            for las_ind in [0, 1]:
                ...
                # get eta0
                curr_eta0 = get_eta0_from_peak_phase_deg(curr_laser, L, scope, curr_phis[las_ind])
                eta0.append(curr_eta0)  # this will be added to the laser dict

                # calculate current laser phase
                curr_eta = get_eta(curr_eta0, L, scope, curr_laser, curr_phis[las_ind])
            ...
            if laser["pol_angle"] == "rra":
                ...
                if diagnostics_on:
                    print(
                        "Laser polarization set to relativistic reversal angle: "
            ...
            if laser["peak_phase_deg"] is not None:  # get laser power [W]
                if laser["peak_phase_deg"] < 0:
                    ...
                    eta0 = get_eta0_from_peak_phase_deg(laser, L, scope, laser_phi)
                    laser.update({"eta0": eta0})
                    laser = get_laser_power_from_eta0(laser, scope["HT"], const)
            ...
            # if power is specified instead of peak phase, get eta0
                ...
                eta0 = get_eta0_from_laser_power(laser, scope["HT"], const)  # [rad]
    ...
    if diagnostics_on:
        if not silent_mode:
            if laser["eta"].std() != 0:
                print("DIAGNOSTICS: Peak laser phase is " "{:.2f} deg.".format(np.rad2deg(eta_max)))
            else:
...
def get_eta(eta0, L, scope, laser, laser_phi):
    ...
    return (
        eta0
        / 2
        * np.exp(-2 * Ly**2 / (1 + Lx**2))
        / np.sqrt(1 + Lx**2)
        * (
            1
            + (1 - 2 * scope["beta_lattice"] ** 2 * (np.cos(np.deg2rad(laser["pol_angle"]))) ** 2)
...
def get_eta0_from_peak_phase_deg(laser, L, scope, laser_phi):
    ...
    peak_phase_deg_test = np.rad2deg(get_eta(eta0_test, L, scope, laser, laser_phi).max())  # [deg]
...
def get_laser_power_from_eta0(laser, e_energy, const, return_power=False, eta0=None):
    if eta0 is None:
...
def get_laser_phase_gradient(L, laser, scope):
    ...
    dLx_dSx = np.cos(np.deg2rad(laser["xy_angle"])) * partial_scale_factor / laser["zR"]
    dLx_dSy = np.sin(np.deg2rad(laser["xy_angle"])) * partial_scale_factor / laser["zR"]
    dLy_dSx = -np.sin(np.deg2rad(laser["xy_angle"])) * partial_scale_factor / laser["w0"]
    dLy_dSy = np.cos(np.deg2rad(laser["xy_angle"])) * partial_scale_factor / laser["w0"]
    ...
    deta_dLx = (
        laser["eta0"]
        / (2 * laser["NA"] ** 2 * (1 + L["x"] ** 2) ** (11 / 4))
        * np.exp(-2 * L["y"] ** 2 / (1 + L["x"] ** 2))
        * (
            laser["NA"] ** 2 * L["x"] * (1 + L["x"] ** 2) ** (1 / 4) * (4 * L["y"] ** 2 - L["x"] ** 2 - 1)
            + np.exp(-2 * np.deg2rad(laser["xz_angle"]) ** 2 * (1 + L["x"] ** 2) / (laser["NA"] ** 2))
            * (
                L["x"]
                * (
                    -4 * np.deg2rad(laser["xz_angle"]) ** 2 * (1 + L["x"] ** 2) ** 2
                    + laser["NA"] ** 2 * (4 * L["y"] ** 2 - 1.5 * (1 + L["x"] ** 2))
                    + scope["beta_lattice"] ** 2
                    * (
                        8 * np.deg2rad(laser["xz_angle"]) ** 2 * (1 + L["x"] ** 2) ** 2
                        + laser["NA"] ** 2 * (3 * (1 + L["x"] ** 2) - 8 * L["y"] ** 2)
                    )
                ...
                * np.cos(
                ...
                + (
                    laser["NA"] ** 2 * (0.75 - L["y"] ** 2 + L["x"] ** 2 * (0.75 + L["y"] ** 2))
                    - 2 * (1 + L["x"] ** 2) * 2
                )
                * ((2 * scope["beta_lattice"] * np.cos(np.deg2rad(laser["pol_angle"]))) ** 2 - 2)
                * np.sin(
                    laser_phi
                    - 4 * L["x"] / (laser["NA"] ** 2)
                    - 2 * L["x"] * L["y"] ** 2 / (1 + L["x"] ** 2)
    ...
    deta_dLy = (
        ...
        * L["y"]
        / ((1 + L["x"] ** 2) ** (7 / 4))
        * np.exp(
            -2 * np.deg2rad(laser["xz_angle"]) ** 2 * (1 + L["x"] ** 2) / (laser["NA"] ** 2)
            - 2 * L["y"] ** 2 / (1 + L["x"] ** 2)
        )
        * (
            (scope["beta_lattice"] ** 2 * (1 + np.cos(2 * np.deg2rad(laser["pol_angle"]))) - 1)
            * (
                np.cos(
                    laser_phi
                    - 4 * L["x"] / (laser["NA"] ** 2)
...
def get_laser_phase_gradient_old(L, laser, scope):
    ...
    deta_dLx = (
        ...
        * (
            L["x"] * (-1 - L["x"] ** 2 + 4 * L["y"] ** 2)
            + 1
            / (laser["NA"] ** 2 * (1 + L["x"] ** 2) ** (1 / 4))
            * (
                laser["NA"] ** 2
                * L["x"]
                * (-1.5 - 1.5 * L["x"] ** 2 + 4 * L["y"] ** 2)
                * np.cos(
                    4 * L["x"] / (laser["NA"] ** 2)
                    + 2 * L["x"] * L["y"] ** 2 / (1 + L["x"] ** 2)
                    - 1.5 * np.arctan(L["x"])
                ...
                + (
                    -4 * (1 + L["x"] ** 2) ** 2
                    + laser["NA"] ** 2 * (1.5 - 2 * L["y"] ** 2 + L["x"] ** 2 * (1.5 + 2 * L["y"] ** 2))
                )
                * np.sin(
                    4 * L["x"] / (laser["NA"] ** 2)
                    + 2 * L["x"] * L["y"] ** 2 / (1 + L["x"] ** 2)
    ...
    deta_dLy = (
        ...
        * (
            (1 + L["x"] ** 2) ** (1 / 4)
            + np.cos(
                4 * L["x"] / (laser["NA"] ** 2) + 2 * L["x"] * L["y"] ** 2 / (1 + L["x"] ** 2) - 1.5 * np.arctan(L["x"])
            )
            + L["x"]
            * np.sin(
                4 * L["x"] / (laser["NA"] ** 2) + 2 * L["x"] * L["y"] ** 2 / (1 + L["x"] ** 2) - 1.5 * np.arctan(L["x"])
...
def get_numerical_phase_plate_gradient(laser, camera, scope):
    ...
    # units are [rad/(A^-1)]
    (deta_dSr, deta_dSc) = np.gradient(
        laser["eta"], 1 / (camera["rows"] * camera["pixel_size"]), 1 / (camera["cols"] * camera["pixel_size"])
    )
    deta_dSx = deta_dSc
...
def move_atom_data_to_origin(atom_data, move_z_only=False):
    ...
    if move_z_only:
        shifted_rcz = (val - np.mean(val) * np.floor(ind / 2) for ind, val in enumerate(rcz))
    else:
...
def get_atom_coords_from_rcz(rcz):
    ...
    for ind in range(len(r)):
        if ind == 0:
            ...
            atom_coords = np.vstack((atom_coords, np.array([r[ind], c[ind], z[ind]])))
    return atom_coords
...
def get_potential_from_atom_data(atom_data, vol_dims_rcz, pix_sizes, pot_pix=11, ss_factor=8, edge_flag=False):
    ...
    atom_pots = [get_atomic_potential(uid, pot_pix, pix_sizes[0], ss_factor) for uid in unique_ids]
    pots_dict = dict(zip(unique_ids, atom_pots))

    if edge_flag:  # pad vol_dims_rcz for large samples (e.g. random_carbon)
        extra_pix = pot_pix
        vol_dims_rcz = vol_dims_rcz + np.array([2 * pix_sizes[0] * extra_pix, 2 * pix_sizes[0] * extra_pix, 0])  # [A]
    ...
    for atom_ind, coords in enumerate(atom_data["coords"]):
        ...
        mini_inds = [
            [int(center_inds[i] - np.floor(pot_pix / 2)), int(center_inds[i] + np.floor(pot_pix / 2)) + 1]
            for i in range(2)
        ]

        # add potential into large fov
        slices = tuple(slice(mini_inds[i][0], mini_inds[i][1]) for i in range(2)) + (center_inds[2],)
    ...
    potential = {"v": vol, "axes": axes, "pix_sizes": pix_sizes}
...
def get_atomic_potential(atom_id, n_grid_pix, pix_size, ss_factor):
    ...
    mini_ax = np.linspace(-pix_size * np.floor(n_grid_pix / 2), pix_size * np.floor(n_grid_pix / 2), n_grid_pix)
    ...
    sup_pot = (
        2 * ap[0] * special.kn(0, 2 * np.pi * sup_rad * np.sqrt(ap[1]))
        ...
        + ap[10] / ap[11] * np.exp(-np.pi**2 * sup_rad**2 / ap[11])
    ) * pot_const
...
def apply_thermal_gaussian(potential, vib_std):
    print("...Applied {} A thermal vibration ".format(vib_std) + "to atomic potential as Gaussian blur.")
...
def get_shang_sigworth_solvent(atom_data, potential, r_asymptote=7.5, r_probe=1.7):
    ...
    [mesh_c, mesh_r, mesh_z] = np.meshgrid(potential["axes"][1], potential["axes"][0], potential["axes"][2])  # [A]
    for atom_ind, coords in enumerate(atom_data["coords"]):
        # get atom location & box around it
        center_inds = [(np.abs(potential["axes"][i] - coords[i])).argmin() for i in range(3)]
        rad_inds = [
            [
                int(center_inds[i] - np.floor(r_asymptote / potential["pix_sizes"][int(i / 2)])),
                int(center_inds[i] + np.floor(r_asymptote / potential["pix_sizes"][int(i / 2)])) + 1,
            ]
            for i in range(3)
        ]
        ...
        if vdw_rad > 0:  # skip atoms with unknown vdw radius?
            ...
            curr_r = (
                np.sqrt(
                    (mesh_r[slices] - coords[0]) ** 2
                    + (mesh_c[slices] - coords[1]) ** 2
                    + (mesh_z[slices] - coords[2]) ** 2
                )
                - vdw_rad
            )
            ...
            id_mat[slices] = np.where(update_dist, atom_data["id"][atom_ind], id_mat[slices])
        if np.mod(atom_ind, 10000) == 0:
            print("......Analyzed " + "{}/{} atoms so far...".format(atom_ind, len(atom_data["coords"])))
    ...
    for ii, ind in enumerate(boundary_inds_T):
        ...
        rad_inds = [
            [
                int(center_inds[i] - np.floor(r_shrink / potential["pix_sizes"][int(i / 2)])),
                int(center_inds[i] + np.floor(r_shrink / potential["pix_sizes"][int(i / 2)])) + 1,
            ]
            for i in range(3)
        ]
        slices = tuple(slice(rad_inds[i][0], rad_inds[i][1]) for i in range(3))
        ...
        if np.mod(ii, 10000) == 0:
            print("......Analyzed " + "{}/{} points so far...".format(ii, len(boundary_inds_T)))
    print("......Analyzed all {} points.".format(len(boundary_inds_T)))
    print("...Masking complete.")
    if diagnostics_on:
        if not silent_mode:
            print("...DIAGNOSTICS: r_shrink has a mean value of " "{:.2f} A.".format(np.mean(r_shrink_test)))
    ...
    r_axis = np.arange(-1 * (np.max(list(vdw_dict.values())) + 1), np.max(rad_dist) + 1, 0.01)
    ...
    density_ss = np.where(id_mat == 6, nonpolar_mat, polar_mat)
    print("...Solvent ready.")
...
def get_shang_sigworth_density(rad_dist, p_np):
    ...
    if p_np == "polar":
        ssp = {"a2": 0.2, "a3": -0.15, "r1": 0.5, "r2": 1.7, "r3": 1.7, "sig1": 1, "sig2": 1.77, "sig3": 1.06}
    elif p_np == "nonpolar":
        ssp = {"a2": 0.15, "a3": -0.12, "r1": 1.0, "r2": 2.2, "r3": 3.6, "sig1": 1, "sig2": 1.77, "sig3": 0.85}
    # eq 1, Shang & Sigworth JSB 2012
    return (
        0.5
        + 0.5 * special.erf((rad_dist - ssp["r1"]) / (np.sqrt(2) * ssp["sig1"]))
...
def get_boundary_inds(bin_mask):
    ...
    for ax_ind in range(3):
        ...
        ax_new = np.where(vals_init < 0, coords_init[ax_ind], coords_init[ax_ind] + 1)  # update axis at vals_init=+1
...
def make_volume_with_axes(vol_dims, pix_sizes, val=0):
    axes = [np.arange(-vol_dims[a] / 2, vol_dims[a] / 2, pix_sizes[int(a / 2)]) for a in range(3)]
    vol = val * np.ones((len(axes[0]), len(axes[1]), len(axes[2])))
    return (vol, axes)


def make_random_atoms(vol_dims_rcz, min_atom_separation, atom_density_gcm3, atom_element):
...
def get_rand_pos(box_xyz_dims):
    return np.array([uniform(0, box_xyz_dims[0]), uniform(0, box_xyz_dims[1]), uniform(0, box_xyz_dims[2])])
...
def calc_3d_pair_corr(atom_coords, r_max, dr, vol_dims_rcz):
    ...
    int_bool = np.logical_and.reduce(
        (
            np.asarray(r_coords) > r_max,
...
def check_atom_coords(atom_coords, min_atom_separation):
    ...
    for atom_id in range(len(r_coords)):
        if atom_id in too_close:
            ...
            # find nearby atoms
            close_atoms_r = set(
                np.asarray(abs(r_coords - r_coords[atom_id]) <= min_atom_separation).nonzero()[0].tolist()
            )
            close_atoms_c = set(
                np.asarray(abs(c_coords - c_coords[atom_id]) <= min_atom_separation).nonzero()[0].tolist()
            )
            close_atoms_z = set(
                np.asarray(abs(z_coords - z_coords[atom_id]) <= min_atom_separation).nonzero()[0].tolist()
            )
            close_atoms_3d = set.intersection(close_atoms_r, close_atoms_c, close_atoms_z)
            ...
            for test_id in close_atoms_3d:
                test_dist = np.sqrt(
                    sum(
                        (
                            np.array([r_coords[atom_id], c_coords[atom_id], z_coords[atom_id]])
                            - np.array([r_coords[test_id], c_coords[test_id], z_coords[test_id]])
                        )
                        ** 2
                    )
                )
    ...
    print("...Finished! " + "There are {} atoms too close together.".format(len(too_close)))
    return too_close


#%% exit wave calculation
def get_exit_wave_op(sample, psi_in, scope, camera, axial_pix_size=1, return_potential=True, atom_data=None):
    pix_sizes = [camera["pixel_size"], axial_pix_size]  # [A]
    volume_dims = [camera["rows"] * pix_sizes[0], camera["cols"] * pix_sizes[0], np.round(sample["thickness"])]  # [A]

    if sample["class"] == "test":
        if sample["name"] == "test_plane":
            test_object = normal(0, sample["test_phase"], (camera["rows"], camera["cols"]))
        elif sample["name"] == "test_circle":
            circle_frac = sample["circle_diam"] / (camera["rows"] * camera["pixel_size"])
            phase_mask = make_aperture_mask(np.ones((camera["rows"], camera["cols"])), circle_frac)
            test_object = sample["test_phase"] * phase_mask
        elif sample["name"] == "test_circle_array":
            lattice_const_pix = int(np.round(sample["circle_lattice_const"] / camera["pixel_size"]))  # [pixels]
            circle_frac = sample["circle_diam"] / (camera["rows"] * camera["pixel_size"])
            circle_mask = make_aperture_mask(np.ones((camera["rows"], camera["cols"])), circle_frac)
            ...
            for row in range(-row_max + 1, row_max):
                for col in range(-col_max + 1, col_max):
                    curr_circle = np.roll(
                        np.roll(circle_mask, lattice_const_pix * row, axis=0), lattice_const_pix * col, axis=1
                    )
        ...
        # make random atoms
        atom_data = make_random_atoms(
            volume_dims, sample["min_atom_sep"], sample["atom_density_gcm3"], sample["atom_element"]
        )
        atom_data = move_atom_data_to_origin(atom_data)

        # calculate potential
        potential = get_potential_from_atom_data(atom_data, volume_dims, pix_sizes, edge_flag=True)
        ...
        atom_data = {
            "coords": np.array([[0, 0, 0]]),  # atom is at origin
            "element": [sample["atom_element"]],
            "id": list(get_id_from_element([sample["atom_element"]])),
        }

        # calculate potential - ensuring single slice
        potential = get_potential_from_atom_data(atom_data, volume_dims, [pix_sizes[0], sample["slice_thickness"]])
        potential.update({"v": np.sum(potential["v"], axis=2)})  # flatten array along z
        ...
        protein_box_dims = get_protein_box_dims(atom_data, pad_size=sample["box_pad_size"])  # [A]
        if diagnostics_on:
            if not silent_mode:
                print("...DIAGNOSTICS: protein box has " + "side length {} A".format(protein_box_dims[0]))
        if np.any(np.asarray(protein_box_dims) > np.asarray(volume_dims)):
            raise Exception("desired volume is too small for protein")
        potential = get_potential_from_atom_data(atom_data, protein_box_dims, pix_sizes)
        if sample["solvated"]:
        ...
        lattice_dims_rcz = np.array([int(lattice_dims[i] / pix_sizes[int(i / 2)]) for i in range(3)])  # [pix]
        ...
        protein_box_dims_rcz = np.array(
            [int(np.ceil(protein_box_dims[i] / pix_sizes[int(i / 2)])) for i in range(3)]  # [pix]
        )
        if np.any(np.asarray(protein_box_dims) > np.asarray(volume_dims)):
        ...
        protein_counts = [int(i) for i in np.asarray(lattice_dims) / np.asarray(protein_box_dims)]
        ...
        for ax_ind, N in enumerate(protein_counts):
            ...
            for box_ind, start_ind in enumerate(start_inds):
                slices[ax_ind, box_ind] = slice(int(start_ind), int(start_ind) + int(curr_d))
        ...
        for row_ind in range(protein_counts[0]):
            for col_ind in range(protein_counts[1]):
                for dep_ind in range(protein_counts[2]):
                    all_box_locs.append(tuple((slices[0, row_ind], slices[1, col_ind], slices[2, dep_ind])))
        print("Created {}/{} boxes.".format(len(all_box_locs), np.prod(protein_counts)))

        if sample["solvated"]:
            (latt_vol, latt_ax) = make_volume_with_axes(lattice_dims, pix_sizes, val=3.6 * axial_pix_size)
        else:
            (latt_vol, latt_ax) = make_volume_with_axes(lattice_dims, pix_sizes, val=0)
        ...
        if sample["interp_positioning"]:
            ...
            stock_potential = get_potential_from_atom_data(atom_data, protein_box_dims, pix_sizes)
            if sample["solvated"]:  # calculate solvent density
                solvent_density = get_shang_sigworth_solvent(atom_data, stock_potential)
            else:
                print("Solvent disabled. Sample is in vacuum.")
                solvent_density = np.zeros_like(stock_potential["v"])  # no solvent
            if sample["vibration"] > 0:  # apply thermal gaussian
                stock_potential = apply_thermal_gaussian(stock_potential, sample["vibration"])
            # eq 4, Shang & Sigworth JSB 2012 - total potential
            stock_potential["v"] = stock_potential["v"] + 3.6 * axial_pix_size * solvent_density
                ...
                if sample["interp_positioning"]:  # do spline interp using scipy
                    ...
                    if sample["random_positions"]:
                        ...
                        if sample["random_position_magnitude"] < 0:
                            ...
                            print("random_position_magnitude cannot be " + "negative, changed to {}".format(magn))
                        curr_pot["v"] = ndimage.shift(
                            curr_pot["v"], uniform(-shift_max, shift_max, (3)), mode="constant", cval=np.mean(face_vals)
                        )
                    ...
                    # calculate potential
                    curr_pot["v"] = get_potential_from_atom_data(atom_data, protein_box_dims, pix_sizes)
                    if sample["solvated"]:  # calculate solvent density
                        solvent_density = get_shang_sigworth_solvent(atom_data, curr_pot)
                    else:  # no solvent
                        print("Solvent disabled. Sample is in vacuum.")
                        solvent_density = np.zeros_like(curr_pot["v"])

                    if sample["vibration"] > 0:  # apply thermal gaussian
                        curr_pot = apply_thermal_gaussian(curr_pot, sample["vibration"])
                    # eq 4, Shang & Sigworth JSB 2012 - total potential
                    curr_pot["v"] = curr_pot["v"] + 3.6 * axial_pix_size * solvent_density
            else:
                curr_pot = stock_potential.copy()
        ...
        if diagnostics_on:
            if not silent_mode:
                print("DIAGNOSTICS: plotting single site potential " + "and lattice potential")
            plt.figure("exit_wave_calc")
            plt.subplot(2, 3, 1)
        ...
        potential = {"v": latt_vol, "axes": latt_ax, "pix_sizes": pix_sizes}
    ...
    if sample["class"] == "test":
        ...
        if sample["class"] == "atom":
            ...
            if not (pix_per_slice.is_integer()):
                raise Exception("slice_thickness not divisible " + "by axial_pix_size")
            (psi_op, psi_exit, track_integral) = do_multislice(
                psi_in, potential, pix_per_slice, pix_sizes[1] * pix_per_slice, scope, camera
            )
        ...
        exit_wave_op = {
            "psi": psi_op,  # refocused wave (for later calcs)
...
def do_multislice(psi_in, potential, pix_per_slice, slice_pix_size, scope, camera):
    ...
    for slc in range(int(n_slices)):
        psi = propagate_slice(psi, get_t_fun(potential, slc, pix_per_slice, scope["sigma_e"]), p_fun)
    ...
    p_fun_back = get_p_fun(scope["wlen"], -1 * slice_pix_size * (n_slices - 1) / 2, S["r"] ** 2, antialias_mask)
    psi = propagate_slice(psi, 1, p_fun_back)
    track_integral.append(np.sum(calc_image_pdf(psi)))

    track_integral = track_integral / track_integral[0]
    print(
        "...Exit wave calculated.\n"
        + "...Wavefunction integral had "
        + "mean {:.3f} and std {:.3f}.".format(np.mean(track_integral), np.std(track_integral))
    )
...
def propagate_slice(psi_in, t_slice, p_fun):
    ...
    return ifft2(
        fft2(psi_in * t_slice, workers=os.cpu_count() - 1) * p_fun, workers=os.cpu_count() - 1
    )  # eq 6.86, Kirkland, preferred


def get_t_fun(potential, slice_ind, pix_per_slice, sigma_e):
    int_pot = np.sum(
        potential["v"][:, :, slice(int(pix_per_slice * slice_ind), int(pix_per_slice * slice_ind + pix_per_slice))],
        axis=2,
    )
...
def make_aperture_mask(r_grid, frac, force_circle=True):
    if force_circle:  # makes circle regardless of aspect ratio
        (_, axes) = make_volume_with_axes([r_grid.shape[0], r_grid.shape[1], 1], [1, 1])
        [mesh_c, mesh_r] = np.meshgrid(axes[1], axes[0])
        rad = np.sqrt(mesh_r**2 + mesh_c**2)
        extreme_val = np.min([np.min(rad[0, :]), np.min(rad[:, 0])])
        mask = np.ones_like(rad)
        mask[rad > extreme_val * frac] = 0
    else:  # makes oval if aspect ratio != 1
        extreme_val = np.min([np.min(r_grid[0, :]), np.min(r_grid[:, 0])])
...
def get_face_vals(cube):
    return np.hstack((cube[0, 0, :], cube[0, :, 0], cube[:, 0, 0], cube[-1, -1, :], cube[-1, :, -1], cube[:, -1, -1]))
...
def embed_protein_box(potential, large_vol_dims):
    ...
    goal_shape = np.array([int(large_vol_dims[i] / potential["pix_sizes"][int(i / 2)]) for i in range(3)])
    pad_amount = (goal_shape - np.asarray(potential["v"].shape)) / 2
    padded_v = np.pad(
        potential["v"],
        tuple(tuple([int(np.ceil(pad_amount[i])), int(np.floor(pad_amount[i]))]) for i in range(3)),
        mode="constant",
        constant_values=np.mean(face_vals),
    )
...
def section_atom_data(atom_data, section_z, section_thickness, rezero_flag=True, z_only_flag=True):
    ...
    if rezero_flag:
        atom_data = move_atom_data_to_origin(atom_data, move_z_only=z_only_flag)
    return atom_data
...
def propagate_bfp_field(psi_in, t_slice, p_fun):
    return fftshift(
        fft2(
            ifft2(ifftshift(psi_in * t_slice), workers=os.cpu_count() - 1) * ifftshift(p_fun),
...
def TEM(exit_wave_op, laser, scope, camera):
    ...
    if laser["z_offset"] != 0:
        raise Exception("This feature is deprecated in" "{}.".format(os.path.basename(__file__)))
...
def downsample_image(im, downsample_factor, mean_flag=False):
    ...
    return np.add.reduceat(
        np.add.reduceat(im, np.arange(0, im.shape[0], downsample_factor), axis=0),
        np.arange(0, im.shape[1], downsample_factor),
        axis=1,
    )
...
def plot_image(im, plot_features=default_plot_features):
    if plot_features["plot_asd_log10_TF"]:
        ...
        im1 = axes[1].imshow(np.log10(asd), cmap="gray", vmin=color_min, vmax=color_max)
        plt.colorbar(im0, cax=mal(axes[0]).append_axes("right", size="5%", pad=0.05))
        plt.colorbar(im1, cax=mal(axes[1]).append_axes("right", size="5%", pad=0.05))
    else:
...
def plot_bfp(laser, camera, scope, plot_features=default_plot_features):
    ...
    im0 = axes[0, 0].imshow(np.abs(scope["H_bfp"]), cmap="gray", extent=2 * [-camera["nyquist"], camera["nyquist"]])
    plt.colorbar(im0, cax=mal(axes[0, 0]).append_axes("right", size="5%", pad=0.05))
    # phase of H_bfp
    axes[0, 1].title.set_text(r"$\operatorname{arg}\left(H_{bfp}\right)$")
    im1 = axes[0, 1].imshow(np.angle(scope["H_bfp"]), cmap="gray", extent=2 * [-camera["nyquist"], camera["nyquist"]])
    cbar1 = plt.colorbar(im1, cax=mal(axes[0, 1]).append_axes("right", size="5%", pad=0.05))
    cbar1.set_label("[rad]")
    ...
    if np.mod(np.round(zf), 2) == 0:
        ...
        zf = np.round(zf)
    h_slc0 = slice(
        int(scope["H_bfp"].shape[0] * np.floor(zf / 2) / zf), -int(scope["H_bfp"].shape[0] * np.floor(zf / 2) / zf)
    )
    h_slc1 = slice(
        int(scope["H_bfp"].shape[1] * np.floor(zf / 2) / zf), -int(scope["H_bfp"].shape[1] * np.floor(zf / 2) / zf)
    )
    axes[0, 2].title.set_text(r"$\operatorname{arg}\left(H_{bfp}\right)$" + " - {}x Zoom".format(zf))
    im2 = axes[0, 2].imshow(
        np.angle(scope["H_bfp"][h_slc0, h_slc1]),
        cmap="gray",
        extent=2 * [-camera["nyquist"] / (2 * zf), camera["nyquist"] / (2 * zf)],
    )
    cbar2 = plt.colorbar(im2, cax=mal(axes[0, 2]).append_axes("right", size="5%", pad=0.05))
    cbar2.set_label("[rad]")
    ...
    if laser["z_offset"] == 0 and laser["eta"].std() != 0:  # laser is in bfp
        # gamma_lattice
        im3 = axes[1, 0].imshow(
            np.angle(np.exp(-1j * scope["gamma_lattice"])),
            cmap="gray",
            extent=2 * [-camera["nyquist"], camera["nyquist"]],
        )
        axes[1, 0].title.set_text(r"$\gamma$")
        cbar3 = plt.colorbar(im3, cax=mal(axes[1, 0]).append_axes("right", size="5%", pad=0.05))
        cbar3.set_label("[rad]")
        ...
        # eta
        im4 = axes[1, 1].imshow(
            np.angle(np.exp(-1j * laser["eta"])), cmap="gray", extent=2 * [-camera["nyquist"], camera["nyquist"]]
        )
        axes[1, 1].title.set_text(r"$\eta$")
        cbar4 = plt.colorbar(im4, cax=mal(axes[1, 1]).append_axes("right", size="5%", pad=0.05))
        cbar4.set_label("[rad]")
        ...
        # gamma_lattice + eta
        im5 = axes[1, 2].imshow(
            np.angle(np.exp(-1j * (scope["gamma_lattice"] + laser["eta"]))),
            cmap="gray",
            extent=2 * [-camera["nyquist"], camera["nyquist"]],
        )
        axes[1, 2].title.set_text(r"$\gamma + \eta$")
        cbar5 = plt.colorbar(im5, cax=mal(axes[1, 2]).append_axes("right", size="5%", pad=0.05))
        cbar5.set_label("[rad]")
...
def plot_lpp(laser, camera, scope, plot_features=default_plot_features):
    if laser["eta"].std() == 0:
        ...
        if np.mod(np.round(zf), 2) == 0:
            ...
            zf = np.round(zf)
        p_slc0 = slice(
            int(phase_full.shape[0] * np.floor(zf / 2) / zf), -int(phase_full.shape[0] * np.floor(zf / 2) / zf)
        )
        p_slc1 = slice(
            int(phase_full.shape[1] * np.floor(zf / 2) / zf), -int(phase_full.shape[1] * np.floor(zf / 2) / zf)
        )

        # figure setup
        fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(12, 7))
        fig.suptitle("Laser Phase Profile ($\eta_{max}$ = " + "{:.2f}$^o$)".format(np.rad2deg(laser["eta0"])))
        ...
        im0 = axes[0].imshow(
            ...
            cmap="gist_heat_r",
            vmin=0,
            vmax=-np.ceil(np.rad2deg(laser["eta0"])),
            extent=2 * [-camera["nyquist"], camera["nyquist"]],
        )
        axes[0].title.set_text("Whole BFP")
        axes[0].set_xlabel("[1/$\AA$]")
        cbar0 = plt.colorbar(im0, cax=mal(axes[0]).append_axes("right", size="5%", pad=0.05))
        cbar0.set_label("[deg]")
        ...
        cbar1 = plt.colorbar(im1, cax=mal(axes[1]).append_axes("right", size="5%", pad=0.05))
        cbar1.set_label("[deg]")
...
def plot_ctf(img, laser, camera, scope, plot_features=default_plot_features):
    # preliminary calculations
    S = make_bfp_coords_S(camera)
    (ctf, atf) = get_transfer_func(img, scope, camera, laser, ctf_mode=True)
    (A, xi, nA, nxi) = get_transfer_func(img, scope, camera, laser, ctf_mode=True, return_ctf_components=True)
    if plot_features["astig_avg_TF"] or img["astig_z"] != 0:
        f_tot = make_defocus_coord(S, img)
        (ctf_radial, _) = calc_radial_profile(np.real(ctf), r_map=f_tot * S["r"], n_bins=plot_features["ctf_bins"])
        (atf_radial, _) = calc_radial_profile(np.real(atf), r_map=f_tot * S["r"], n_bins=plot_features["ctf_bins"])
    else:
        (ctf_radial, _) = calc_radial_profile(np.real(ctf), r_map=S["r"], n_bins=plot_features["ctf_bins"])
        (atf_radial, _) = calc_radial_profile(np.real(atf), r_map=S["r"], n_bins=plot_features["ctf_bins"])
    radial_axis = np.linspace(S["r"].min(), S["r"].max(), num=plot_features["ctf_bins"])

    fig, axes = plt.subplots(nrows=2, ncols=3, figsize=(12, 7))

    # A (magnitude of H_bfp)
    axes[0, 0].title.set_text(r"$A = \left|H_{bfp}\right|$")
    im0 = axes[0, 0].imshow(A, cmap="gray", vmin=0, vmax=1, extent=2 * [-camera["nyquist"], camera["nyquist"]])
    plt.colorbar(im0, cax=mal(axes[0, 0]).append_axes("right", size="5%", pad=0.05))

    # xi (phase of H_bfp)
    axes[1, 0].title.set_text(r"$\xi = \operatorname{arg}\left(H_{bfp}\right)$")
    im1 = axes[1, 0].imshow(xi, cmap="gray", extent=2 * [-camera["nyquist"], camera["nyquist"]])
    cbar1 = plt.colorbar(im1, cax=mal(axes[1, 0]).append_axes("right", size="5%", pad=0.05))
    cbar1.set_label("[rad]")
    ...
    im2 = axes[0, 1].imshow(
        np.real(ctf), cmap="gray", vmin=0, vmax=1, extent=2 * [-camera["nyquist"], camera["nyquist"]]
    )
    cbar2 = plt.colorbar(im2, cax=mal(axes[0, 1]).append_axes("right", size="5%", pad=0.05))
    cbar2.set_ticks([0, 1])
    ...
    im3 = axes[1, 1].imshow(
        np.real(atf), cmap="gray", vmin=0, vmax=1, extent=2 * [-camera["nyquist"], camera["nyquist"]]
    )
    cbar3 = plt.colorbar(im3, cax=mal(axes[1, 1]).append_axes("right", size="5%", pad=0.05))
    cbar3.set_ticks([0, 1])
...
def save_plot(plot_features, save_name):
    if plot_features["save_plots_TF"]:
        if not os.path.isdir(plot_features["save_folder_name"]):
            os.mkdir(plot_features["save_folder_name"])
        plt.savefig(os.path.join(plot_features["save_folder_name"], save_name + ".png"), bbox_inches="tight")
