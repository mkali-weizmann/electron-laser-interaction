...
class Seminar(Slide):  # , ZoomedScene  # MovingCameraScene
    ...
    def phase_estimation_examples_scene(self):
        # self.ligo_scene()
        pass
    ...
    def limitations_scene(self):
        ...
        self.next_slide()
        self.play(FadeOut(mirror_u, mirror_d, beam_splitter_r, beam_splitter_l, detector_1, detector_2, label_1, label_2, *beams, column_1_v1, column_2_v1, label_43_v1, label_57_v1, curved_arrow, label_shot_noise, classical_behavior, title_limitations))

    def quantum_metrology_scene(self):
        title_quantum_metrology = Tex("Quantum Metrology", color=WHITE).to_edge(UP)
        quantum_metrology_definition = Text("Quantum metrology is the study of utilizing quantum\nproperties of systems (entanglement, squeezing) to improve\nvarious measurement sensitivities",).scale(0.7)
    ...
    def multiple_phases_estimation_scene(self):
    ...
    def image_while_I_drink(self):
        ...
        self.play(FadeIn(image_while_I_drink, explanation_text, shift=8*DOWN), run_time=0.2)
        self.play(explanation_text.animate.to_edge(DOWN), run_time=10, rate_func=linear)

        self.play(FadeOut(image_while_I_drink, explanation_text, shift=8*UP), run_time=0.2)
    ...
    def multiple_phases_probe_state(self):
        ...
        noon_tex_v5 = MathTex(r"\alpha", r"\left|N,0,0,\ldots,0\right\rangle ", "+", r"\beta", r"\left(\left|0,N,0,\ldots0\right\rangle +\left|0,0,N,\ldots0\right\rangle +\ldots\left|0,0,0,\ldots N\right\rangle \right)").scale(0.7)
        definitions_tex_1 = MathTex(r"\alpha^2+\beta^2=1").next_to(noon_tex_v5, DOWN).shift(5*LEFT).scale(0.5)
        ...
        minimal_error_value_real_v1 = MathTex(r"\left|\Delta\theta\right|^{2}=", r"\frac{\left(1+\sqrt{d}\right)^{2}\frac{d^{2}}{4}}{N^{2}}", r"\ ").scale(0.5).next_to(alpha_value, DOWN)
        minimal_error_value_real_v2 = MathTex(r"\left|\Delta\theta\right|^{2}=", r"\frac{\left(1+\sqrt{d}\right)^{2}\frac{d^{2}}{4}}{N^{2}}", r"\propto\frac{d^{2}}{N^{2}}").scale(0.5).next_to(alpha_value, DOWN)
    ...
    def hb_states_scene(self):
        ...
        problem_text = Text("The Noon state are difficult to realize experimentally. - It requires non-linear interactions").next_to(hb_title, DOWN).scale(0.5)
        ...
        right_creation_operator_up = MathTex(r"A_{l}^{\dagger}=C\sum_{k}a_{k}^{\dagger}e^{-i2\pi\frac{k \cdot l}{d}}").next_to(lines[0], UP).scale(0.5).align_to(lines[2], LEFT)
    ...
    def conclusion_scene(self):
        ...
        self.play(FadeIn(alexeys_image), run_time=5, rate_func=linear)

    def construct(self):
        title_intro = Tex("Improving phase measurement sensitivity beyond the NOON state", color=WHITE).to_edge(
            UP).scale(0.7)
        ...
        introducing_text = Text("When do we need to estimate a phase?", color=WHITE).scale(0.6).next_to(
            title_phase_estimation, DOWN).shift(1.2 * DOWN).to_edge(LEFT)

        first_usage_text = Text("When trying to detect gravitational waves").scale(0.6).next_to(introducing_text,
                                                                                                DOWN).to_edge(
            LEFT).shift(DOWN + RIGHT)
        second_usage_text = Text("When trying to image phase objects").scale(0.6).next_to(first_usage_text,
                                                                                          DOWN).align_to(
            first_usage_text, LEFT).shift(0.4 * DOWN)
        third_usage_text = Text("When trying to impress a pretty woman").scale(0.6).next_to(second_usage_text,
                                                                                            DOWN).align_to(
            second_usage_text, LEFT).shift(0.4 * DOWN)
        ...
        self.next_slide()
        self.play(
            FadeOut(title_phase_estimation, introducing_text, first_usage_text, second_usage_text, third_usage_text,
                    bullet_1, bullet_2, bullet_3))
        #
        # self.phase_estimation_examples_scene()
        #
        ...
        #
        # self.multiple_phases_estimation_scene()
        #
